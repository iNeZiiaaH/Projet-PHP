...
if (isset($_GET['id'])) {
    ...
    if ($stmt1 = $stmt->fetch()) {
        ...
        $pdf->Ln();
        $pdf->Cell(0, 10, 'Total : ' . number_format($stmt1['total'], 2, ',', ' ') . chr(128));
        $pdf->Ln();
        $pdf->Cell(0, 10, 'Commentaire : ' . mb_convert_encoding($stmt1['commentaire'], 'ISO-8859-2', 'UTF-8'));
        $pdf->Ln();
        ...
        while ($row = $stmt2->fetch()) {
            $pdf->Cell(40, 10, mb_convert_encoding($row['description'], 'ISO-8859-2', 'UTF-8'));
            $pdf->Cell(30, 10, $row['quantite']);
            $pdf->Cell(30, 10, number_format($row['prix_unitaire'], 2, ',', ' ' ) . chr(128));
            $total_ligne = $row['quantite'] * $row['prix_unitaire'];
            $pdf->Cell(30, 10, number_format($total_ligne, 2, ',', ' ') . chr(128));
        ...
        $pdf->Cell(30, 10, number_format($total_facture, 2, ',', ' ') . chr(128));
